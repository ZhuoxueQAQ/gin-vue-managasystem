<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="培训项目表格模板" prop="projectTableCol" width="120" />
        <el-table-column align="left" label="收入流水表格模板" prop="incomeStreamTableCols" width="120" />
        <el-table-column align="left" label="支出流水表格模板" prop="outcomeStreamTableCols" width="120" />
        <el-table-column align="left" label="培训项目表单校验规则" prop="projectFormRules" width="120" />
        <el-table-column align="left" label="收入流水表单校验规则" prop="incomeStreamFormRules" width="120" />
        <el-table-column align="left" label="支出流水表单校验规则" prop="outcomeStreamFormRules" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateManageSystemSettingFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteManageSystemSetting,
  deleteManageSystemSettingByIds,
  updateManageSystemSetting,
  findManageSystemSetting,
  getManageSystemSettingList
...
const getTableData = async() => {
  const table = await getManageSystemSettingList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteManageSystemSettingFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteManageSystemSettingByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateManageSystemSettingFunc = async(row) => {
    const res = await findManageSystemSetting({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteManageSystemSettingFunc = async (row) => {
    const res = await deleteManageSystemSetting({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
